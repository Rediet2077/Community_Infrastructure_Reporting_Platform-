...
class ApiService {
  ...
  static Future<Map<String, dynamic>> getReports({
    ...
    try {
      ...
      final uri = Uri.parse(ApiConstants.reportsUrl).replace(queryParameters: queryParams);
      final response = await http.get(uri, headers: headers).timeout(ApiConstants.connectTimeout);
      final data = jsonDecode(response.body);
  ...
  static Future<Map<String, dynamic>> createReport({
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        if (errMsg.isEmpty) errMsg = 'Failed to create report (${response.statusCode})';
        return {'success': false, 'message': errMsg, 'errors': data};
      }
    } catch (e) {
      return {'success': false, 'message': 'Network error: $e'};
    }
