...
class AppState extends ChangeNotifier {
  ...
  void add(Product product) {
    _cart[product.id] = CartItem(product, (_cart[product.id]?.quantity ?? 0) + 1);
    _saveCart(); notifyListeners();
  }

  void setQuantity(Product product, int quantity) {
    if (quantity <= 0) { _cart.remove(product.id); } else { _cart[product.id] = CartItem(product, quantity); }
    _saveCart(); notifyListeners();
  }

  Future<void> _saveCart() async {
    final p = await SharedPreferences.getInstance();
    await p.setString('cart', jsonEncode(cart.map((e) => e.toJson()).toList()));
  }

  Future<String?> login(String username, String password) async {
    try {
      token = await _api.login(username.trim(), password);
      final p = await SharedPreferences.getInstance();
      await p.setString('token', token!);
      await loadProfile(); notifyListeners(); return null;
    } on ApiException catch (e) { return e.message; }
  }

  Future<void> loadProfile() async {
    loadingProfile = true; notifyListeners();
    try { profile = await _api.user(1); }
    on ApiException catch (e) { error = e.message; }
    finally { loadingProfile = false; notifyListeners(); }
  }

  Future<void> logout() async {
    token = null; profile = null;
    final p = await SharedPreferences.getInstance();
    await p.remove('token'); notifyListeners();
  }
}
